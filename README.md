...
The above 5 smart contracts, have been succesfully deployed.
...
flow transactions send ./cadence/transactions/lockUps/createLockUp.cdc 1700034523.0 $RECIPIENT_ADDRESS --network=emulator --signer=holder
...
We proceed by setting up the `holder` account with a `ExampleNFT` collection.

```bash
# With this NodeJS script, we properly setup the holder account.
node cadence/transactions/lockUps/initCollection.js $EXAMPLE_NFT_IDENTIFIER holder
