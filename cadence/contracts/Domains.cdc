...
pub contract Domains: NonFungibleToken {
    ...
    init() {
        ...
        // This Capability only allows for calling these methods: deposit() / getIDs() / borrowNFT() / borrowDomain()
        ...
        // This Capability allows calling all methods provided by the Domains.Collection resource interface
        self.account.link<&Domains.Collection>(self.DomainsPrivatePath, target: self.DomainsStoragePath)

        // Fetch the private Capability for the Domains.Collection resource interface
        let collectionCapability = self.account.getCapability<&Domains.Collection>(self.DomainsPrivatePath)
        // Create the FungibleToken.Vault resource
        ...
        // This Capability only allows for calling these methods: renewDomain() / registerDomain() / getPrices() / getVaultBalance()
        self.account.link<&Domains.Registrar{Domains.RegistrarPublic}>(self.RegistrarPublicPath, target: self.RegistrarStoragePath)
        // This Capability allows calling all methods provided by the Domains.Registrar resource interface
    ...
    pub resource Collection: CollectionPublic, CollectionPrivate, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic {
        ...
        // for withdrawing a token from a collection
        ...
        // for depositing a token to a collection
        pub fun deposit(token: @NonFungibleToken.NFT) {
            // Typecast the token to a Domains.NFT resource
        ...
        // Returns a type-casted Domain resource, instead of just an NFT
        pub fun borrowDomain(id: UInt64): &{Domains.DomainPublic} {
            pre {
        ...
        access(account) fun mintDomain(name: String, nameHash: String, expiresAt: UFix64, receiver: Capability<&{NonFungibleToken.Receiver}>){
            ...
            // Borrow the Receiver resource in order to deposit the token
    ...
    pub resource Registrar: RegistrarPublic, RegistrarPrivate {
        ...
        pub fun registerDomain(name: String, duration: UFix64, feeTokens: @FungibleToken.Vault, receiver: Capability<&{NonFungibleToken.Receiver}>) {
            ...
            // Borrow the private Domains.Collection capability and mint a new Domain token
    ...
    pub fun getPrices(): {Int: UFix64} {
        ...
        let registrar = cap.borrow() ?? panic("Could not borrow collection")
